package com.calendarengine.modules.sync.actions

import com.calendarengine.config.SyncConfig
import com.calendarengine.models.OAuthTokens
...
class GoogleOAuthAction(
    ...
    suspend fun exchangeCode(code: String, tenantId: Int, redirectUri: String): OAuthTokenResult {
        ...
        transaction {
            // Upsert: delete existing token for this tenant+provider, then insert
            OAuthTokens.deleteWhere {
                (OAuthTokens.tenantId eq tenantId) and (OAuthTokens.provider eq "GOOGLE")
            }
    ...
    suspend fun getValidAccessToken(tenantId: Int): String {
        val token = transaction {
            OAuthTokens.selectAll()
                .where { (OAuthTokens.tenantId eq tenantId) and (OAuthTokens.provider eq "GOOGLE") }
                .firstOrNull()
